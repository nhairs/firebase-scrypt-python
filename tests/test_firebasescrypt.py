...
## Application
from firebase_scrypt import generate_derived_key, verify_password
...
def test_verifies_password_correctly():
    result = verify_password(
        password, password_hash, salt, salt_separator, signer_key, rounds, mem_cost
    )
    assert result
...
def test_fails_with_invalid_password():
    ...
    result = verify_password(
        invalid_password, password_hash, salt, salt_separator, signer_key, rounds, mem_cost
    )
    assert not result
...
def test_generate_derived_key():
    ...
    expected_key = (
        "6H+iLZtOO+a71BIU8vmPjHi2lL0X4Swrc1AQVKIJnOEf6JZIPGikQ8bPn/io3+Hf4q2qS+bIyht2hmh6JvSIMQ=="
    )
